...
if [ -z "${failure_reason}" ]; then
    if node -e "const ok = Number(process.argv[1]); const gr = Number(process.argv[2]); const runtime = Number(process.argv[3]); process.exit(ok >= 0.70 && gr >= 0.90 && runtime <= 60 ? 0 : 1)" "${ok_win_rate}" "${gr_win_rate}" "${ok_runtime_sec}"; then
