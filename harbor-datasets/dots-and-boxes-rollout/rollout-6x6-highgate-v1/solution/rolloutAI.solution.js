...
RolloutAI.prototype.sampleSafeEdges=function(gameData){
    ...
    var result=[]
    var seen={}
    var frontCount=Math.min(12,this.safeCandidateLimit,annotated.length)
...
RolloutAI.prototype.pickSafeEdge=function(gameData){
    ...
    var rolloutCount=4
    if(remaining<=28 && candidates.length<=18)rolloutCount=5
    if(remaining<=20 && candidates.length<=14)rolloutCount=6
    if(remaining<=12 && candidates.length<=8)rolloutCount=8
...
RolloutAI.prototype.pickSingleRegionMove=function(gameData){
    ...
    var rolloutCount=8
    if(remaining<=6)rolloutCount=12
    var seedBase=this.getRolloutSeedBase(gameData)
